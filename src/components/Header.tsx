  ...
  return (
    <header className="sticky top-0 z-50">
      ...
      <div className="relative bg-gradient-to-br from-navy via-navy/95 to-navy overflow-hidden">
        ...
        <div className="container mx-auto py-2 md:py-4 px-3 md:px-4 relative">
          ...
          <div className="flex flex-col items-center gap-1 md:gap-2">
            ...
            <div className="flex items-center justify-center gap-3 md:gap-8">
              ...
              <div className="flex flex-col items-center">
                <img
                  ...
                  className="h-10 md:h-20 w-auto object-contain drop-shadow-2xl"
                />
              </div>
            ...
            <div className="flex items-center gap-1.5 md:gap-2">
              <Sparkles className="h-2.5 w-2.5 md:h-3 md:w-3 text-gold" />
              <p className="text-gold font-display font-semibold text-[10px] md:text-sm tracking-wide">
                Celebrating 50+ Years of Sweet Legacy
              </p>
              <Sparkles className="h-2.5 w-2.5 md:h-3 md:w-3 text-gold" />
            </div>

            {/* Product Highlights - More compact on mobile */}
            <div className="flex items-center justify-center gap-1 md:gap-2 flex-wrap py-1.5 md:py-2 border-y border-gold/20 w-full">
              {productHighlights.map((item, index) => (
                <span key={item} className="flex items-center">
                  <span className="text-[10px] md:text-sm font-display font-bold text-cream tracking-wide whitespace-nowrap">
                    {item}
                  </span>
                  {index < productHighlights.length - 1 && (
                    <span className="text-gold font-bold mx-1 md:mx-2 text-[8px] md:text-xs">✦</span>
                  )}
                </span>
              ))}
            </div>

            {/* Contact Section - Simplified on mobile */}
            <div className="flex items-center gap-2 md:gap-4">
              {/* Address - Icon only on mobile */}
              <button
                onClick={handleLocationClick}
                className="flex items-center gap-1.5 text-cream hover:text-gold transition-colors group active:scale-95"
                aria-label="View location on map"
              >
                <MapPin className="h-4 w-4 md:h-3.5 md:w-3.5 text-gold" />
                <span className="hidden md:inline text-xs font-bold">
                  Near Infosys, Pocharam, Ghatkesar
                </span>
              </button>
              ...
              <button
                ...
                className="flex items-center gap-1.5 bg-[#25D366] hover:bg-[#20BD5A] text-white px-3 py-1.5 rounded-full transition-all active:scale-95 shadow-md shadow-[#25D366]/30 font-bold text-xs"
              >
                <svg
                  viewBox="0 0 24 24"
